...
def build_html_email(qa_jobs, url):
    ...
    for job in qa_jobs:
        job_cards += (
            ...
            f'<p style="margin:0 0 6px 0;font-size:17px;font-weight:600;color:#1a1a2e;">💼 {job["title"]}</p>'
            f'<a href="{job["url"]}" style="display:inline-block;margin-top:10px;padding:8px 20px;'
            'background:linear-gradient(135deg,#6c63ff,#48c6ef);color:#fff;text-decoration:none;'
            'border-radius:20px;font-size:13px;font-weight:600;letter-spacing:0.5px;">View &amp; Apply →</a>'
            '</div>'
        )
    ...
    html = (
        ...
        f'{count} new opening{plural} found at Xello · {today}</p>'
        '</div>'
        ...
        f'QA-related opening{plural} matching your alert:</p>'
        + job_cards +
        '</div>'
        ...
        'font-size:14px;font-weight:600;">View All Xello Jobs</a>'
        '</div>'

        '<div style="background:#eef0ff;padding:18px 28px;text-align:center;border-top:1px solid #dde1ff;">'
        '<p style="margin:0;color:#999;font-size:12px;line-height:1.6;">'
        'Powered by GitHub Actions · Runs twice daily at 9:00 AM &amp; 5:00 PM IST</p>'
        '</div>'
...
def build_weekly_summary_email(url, checks_done, qa_jobs_found):
    ...
    html = (
        ...
        f'<p style="margin:8px 0 0;color:rgba(255,255,255,0.85);font-size:14px;">'
        f'Your job alert bot is alive and running · {today}</p>'
        '</div>'

        '<div style="background:#f7f8ff;padding:32px 28px;">'
        '<p style="margin:0 0 24px;color:#555;font-size:14px;line-height:1.6;">'
        'Here\'s a summary of what your job alert bot did this week:</p>'
        '<div style="display:flex;gap:12px;justify-content:center;">'

        '<div style="background:#fff;border-radius:12px;padding:16px 24px;text-align:center;'
        'border:1px solid #e0e4ff;flex:1;">'
        f'<p style="margin:0;font-size:28px;font-weight:700;color:#6c63ff;">{checks_done}</p>'
        '<p style="margin:4px 0 0;font-size:12px;color:#888;">Checks done</p>'
        '</div>'

        '<div style="background:#fff;border-radius:12px;padding:16px 24px;text-align:center;'
        'border:1px solid #e0e4ff;flex:1;">'
        f'<p style="margin:0;font-size:28px;font-weight:700;color:#43b89c;">{qa_jobs_found}</p>'
        ...
        f'<a href="{url}" style="display:inline-block;padding:12px 32px;'
        'background:#1a1a2e;color:#fff;text-decoration:none;border-radius:25px;'
        'font-size:14px;font-weight:600;">View Xello Jobs Board</a>'
        '</div>'

        '<div style="background:#eef0ff;padding:18px 28px;text-align:center;border-top:1px solid #dde1ff;">'
        '<p style="margin:0;color:#999;font-size:12px;line-height:1.6;">'
        'This weekly summary is sent every Sunday · Powered by GitHub Actions</p>'
        '</div>'
...
def build_error_email(error_message, run_time):
    html = (
        ...
        f'<p style="margin:8px 0 0;color:rgba(255,255,255,0.85);font-size:14px;">Detected at {run_time}</p>'
        '</div>'
        ...
        'Please review the error below:</p>'
        '<div style="background:#fff;border-radius:10px;padding:16px 20px;border-left:4px solid #e63946;'
        f'font-family:monospace;font-size:13px;color:#c1121f;word-break:break-word;">{error_message}</div>'
        ...
        'The bot will retry automatically on its next scheduled run.</p>'
        '</div>'

        '<div style="background:#eef0ff;padding:18px 28px;text-align:center;border-top:1px solid #dde1ff;">'
        '<p style="margin:0;color:#999;font-size:12px;">'
        'Powered by GitHub Actions · Check the Actions tab for full logs</p>'
        '</div>'
